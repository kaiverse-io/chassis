...
- **lean-ctx** ([yvgude/lean-ctx](https://github.com/yvgude/lean-ctx)) — context-compression MCP
  layer; gates what gets read, caches re-reads, targets 60-90% token savings (`lean-ctx gain`).
  More invasive than the rest of this cockpit — it sits between the agent and its context, not
  just observing it. Installed via `npm install -g lean-ctx-bin` (building its Rust source needs
  edition-2024/rustc 1.85+ and OOM's small containers via LTO — npm ships the prebuilt binary).
  Its MCP tools (`ctx_read`/`ctx_shell`/`ctx_search`/etc.) need a session restart after install to
  appear — see the tool-preference table below.
...
- `/dev-coach` — anti-pattern detection + AGENTS.md auditor, specifically for Claude Code
  ...
  (full-transcript correction search) and `lean-ctx` (repeat-read hotspots) when present. Run
...
## Token frugality — tool preference

This applies to **any** AI coding agent working in this repo, not just Claude Code — the rule is
conditional, not tool-specific: **if the tools below are listed as available in your current
session, prefer them over their native equivalents; if they are not listed, use native tools
throughout.** MCP tool availability depends on each agent's own session/config, not on this file.
...
| `ctx_edit(path, old, new)` | `Edit` (when native Read is unavailable) | Search-and-replace without a native Read first |

`ctx_read` modes: `auto` (recommended default), `full` (files you're about to edit), `map`
(deps/exports, context-only), `signatures` (API surface only), `diff` (changed lines after an
edit), `aggressive` (max compression, context-only), `entropy` (high-entropy fragments),
`task` (task-filtered), `reference` (quote-friendly excerpts), `lines:N-M` (specific range).

Editing: use native `Edit`/`StrReplace` when available. If `Edit` requires a prior native `Read`
and `Read` is unavailable, use `ctx_edit` — never loop on repeated `Edit` failures, switch
immediately. `Write`/`Delete`/`Glob` — use normally regardless.

Proactive, without being asked: run an overview-equivalent (`ctx_overview` / `graphify-out/GRAPH_REPORT.md`)
at the start of non-trivial work in unfamiliar code, and compress (`ctx_compress`) when context
grows large mid-session.
<!-- cockpit-section:end -->
